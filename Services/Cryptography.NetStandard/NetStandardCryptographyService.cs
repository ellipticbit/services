...
namespace EllipticBit.Services.Cryptography
{
	internal class NetStandardCryptographyService : ICryptographyService
	{
		...
		public NetStandardCryptographyService(NetStandardCryptographyServiceOptions options) {
		...
		public byte[] Decrypt(byte[] key, string data) {
			...
			if (encryptionAlgorithm == EncryptionAlgorithm.AES128GCM || encryptionAlgorithm == EncryptionAlgorithm.AES192GCM || encryptionAlgorithm == EncryptionAlgorithm.AES256GCM) {
				throw new ArgumentOutOfRangeException("encryptionAlgorithm", "GCM Modes not supported in .NET Standard 2.0");
			}

			if (key.Length != 16 && (encryptionAlgorithm == EncryptionAlgorithm.AES128CBC)) throw new ArgumentOutOfRangeException("Invalid key size. Key must be 16 bytes.");
			if (key.Length != 24 && (encryptionAlgorithm == EncryptionAlgorithm.AES192CBC)) throw new ArgumentOutOfRangeException("Invalid key size. Key must be 24 bytes.");
			if (key.Length != 32 && (encryptionAlgorithm == EncryptionAlgorithm.AES256CBC)) throw new ArgumentOutOfRangeException("Invalid key size. Key must be 32 bytes.");
